...
(function () {
  ...
  function paintBranding() {
    // Ministry of Textiles emblem (bundled at project root as ministry-emblem.webp).
    document.querySelectorAll("[data-brand-mark]").forEach(function (el) {
      el.innerHTML = '<img src="ministry-emblem.webp" alt="Ministry of Textiles emblem" width="52" height="52">';
    });
    document.querySelectorAll("[data-flag]").forEach(function (el) { el.innerHTML = flagSVG(); });
    // Digital India logo (bundled at project root as digital-india-logo.png).
    document.querySelectorAll("[data-digital-india]").forEach(function (el) {
      el.innerHTML = '<img class="digital-logo" src="digital-india-logo.png" alt="Digital India — Power to Empower">';
    });
  ...
  /* =================================================================
     LANDING PAGE
  ================================================================= */
  var SESSION_KEY = "textilePortalSession";

  function initLanding() {
    var loginBtn = document.getElementById("loginBtn");
    if (!loginBtn) return;

    // If someone lands here still logged in, clear the stale session so the
    // login form is always the real gate.
    clearSession();
    ...
    ["beneId", "benePass"].forEach(function (fid) {
      ...
      if (f) f.addEventListener("keydown", function (e) { if (e.key === "Enter") attemptLogin(); });
    });

    // Clickable role cards no longer bypass login — they just pre-select the
    // category in the login box and take the user straight to the credential
    // fields, since every dashboard now requires signing in first.
    document.querySelectorAll("[data-goto]").forEach(function (card) {
      card.addEventListener("click", function () { preselectRole(card.getAttribute("data-goto")); });
      card.addEventListener("keydown", function (e) {
        if (e.key === "Enter" || e.key === " ") { e.preventDefault(); preselectRole(card.getAttribute("data-goto")); }
      });
    });
  }
  ...
  function preselectRole(role) {
    ...
    var idField = document.getElementById("beneId");
  ...
  function attemptLogin() {
    ...
    var pass = (document.getElementById("benePass").value || "").trim();
    var roleCfg = DATA.roles[role];

    if (!roleCfg) { showLoginError("Please choose a valid category."); return; }
    if (!id || !pass) { showLoginError("Please enter both your Beneficiary ID and password."); return; }

    var creds = roleCfg.login || { username: "demo", password: "demo123" };
    var ok = id.toLowerCase() === String(creds.username).toLowerCase() &&
             pass === String(creds.password);

    if (!ok) {
      showLoginError("Incorrect Beneficiary ID or password for " + roleCfg.label + ". (Demo credentials: " + creds.username + " / " + creds.password + ")");
      return;
    }

    saveSession(role, roleCfg.profile.name);
    go(role, roleCfg.profile.name);
  }
  ...
  function saveSession(role, name) {
    try {
      sessionStorage.setItem(SESSION_KEY, JSON.stringify({ role: role, name: name, loggedIn: true, at: Date.now() }));
    } catch (e) { /* sessionStorage unavailable — dashboard guard will just re-prompt login */ }
  }
  ...
  function initDashboard(role) {
    ...
    // Require an active login session that matches THIS dashboard's role.
    // Anyone opening farmer.html / weaver.html / etc. directly, or a
    // logged-in-as-a-different-role session, gets bounced back to the
    // login page instead of seeing the dashboard.
    var session = readSession();
    if (!session || !session.loggedIn || session.role !== role) {
      window.location.href = "index.html";
      return;
    }

    var params = new URLSearchParams(window.location.search);
    var name = params.get("name") || session.name || cfg.profile.name;
