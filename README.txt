...
Using it:
  • Login is now REQUIRED before any dashboard opens. Clicking a category
    card just pre-selects that category in the login box — it no longer
    jumps straight to the dashboard.
  • Pick a category, enter Beneficiary ID "demo" and Password "demo123",
    then click "Login to dashboard". Wrong credentials show an inline
    error and you stay on the login page.
  • Each role's demo credentials live in js/data.js under
    roles.<role>.login = { username, password } — change these per role,
    or wire this up to a real backend later.
  • The login session is kept in sessionStorage for the browser tab. If
    you open farmer.html / weaver.html / industrialist.html / student.html
    directly without logging in (or logged in as a different role), you
    are bounced back to the login page automatically.
  • Log out clears the session and returns you to the landing page.
